...
ndk::ScopedAStatus RichtapVibrator::init(const std::shared_ptr<IRichtapCallback>& callback) {
    ...
    if (!m_richtap_support) { // can not init serveral times
...
ndk::ScopedAStatus RichtapVibrator::performEnvelope(const std::vector<int32_t>& envInfo,
                                                    bool fastFlag, const std::shared_ptr<IRichtapCallback>& callback) {
    if (m_richtap_support) {
        if (envInfo.size() == 0) {
...
ndk::ScopedAStatus RichtapVibrator::performHeParam(int32_t interval, int32_t amplitude, int32_t freq,
                                                   const std::shared_ptr<IRichtapCallback>& callback) {
    if (m_richtap_support) {
        if (DEBUG_ENABLED) {
